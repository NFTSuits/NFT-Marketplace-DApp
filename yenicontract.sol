...
contract nftContract is ERC721Full {
    ...
    function removeFromAvatars(uint index) internal {
        require(index < Avatars.length);
        Avatars[index] = Avatars[Avatars.length-1];
        Avatars.length--;
    }
    ...
    function wearItems(
        ...
        if( users[msg.sender].avatarIndex && (_headTokenId == 0 || _middleTokenId == 0 || _bottomTokenId == 0)){
            removeFromAvatars(users[msg.sender].avatarIndex);
            users[msg.sender].avatarIndex = 0;
        }
        
        if(_headTokenId != 0 && _middleTokenId != 0 && _bottomTokenId != 0){
            if(users[msg.sender].avatarIndex){
                removeFromAvatars(users[msg.sender].avatarIndex);
            }
            uint256 _id = Avatars.push(avatar(users[msg.sender].head, users[msg.sender].middle!= 0, users[msg.sender].bottom));
            users[msg.sender].avatarIndex = _id;
        }
    ...
    function wearItem(uint256 _tokenId) public {
        ...
        if(users[msg.sender].head != 0 && users[msg.sender].middle!= 0 && users[msg.sender].bottom != 0){
            if(users[msg.sender].avatarIndex){
                removeFromAvatars(users[msg.sender].avatarIndex);
            }
            uint256 _id = Avatars.push(avatar(users[msg.sender].head, users[msg.sender].middle!= 0, users[msg.sender].bottom));
